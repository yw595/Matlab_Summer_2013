...
%
function [v_sol, corrval, nvar] = eMOMAYiping(m,r,r_sd,r_group,gene_to_scale,minFit,upt_const)
%
...
while sum(~m.rev) > nR_old
    ...
    N = spalloc(nmets + 1 + 2*nnnan, nrxns + 2 + nnnan_nrev, floor(2.1*nSnz));
    %disp('size N:');
    %disp(size(N));
