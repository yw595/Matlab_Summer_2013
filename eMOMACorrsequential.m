...
models=constrainexchangesequential(rec2);
for i=1:length(models)
    %if(~strcmp(celllinesarray{i},'MDA-MB-468') && ~strcmp(celllinesarray{i},'RXF 393'))
    tempmodel=models{i};
    expressionFile='UACC_257';
    outputFile=['../eMOMACorroutconstrainedsequentialmediumuptakenarayanbetainescale/' expressionFile num2str(i) 'out'];
    ...
    if(i==1)
        ...
        ithmodel=models{i};
        glcexcind=find(ismember(ithmodel.rxns,'EX_glyb(e)'));
        glcexclb=ithmodel.lb(glcexcind);
        glcexclb=glcexclb(1);
        [v_solirrev v_solrev]=runeMOMA(models{i},expressionFileLoc,{{'pyruvate kinase'},{abs(glcexclb)}});
    end
